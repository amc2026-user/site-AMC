...
const api = {
  ...
  createOffer: (data: Omit<Offer, "id" | "createdAt">) =>
    fetch(`${API}/offers`, { method: "POST", headers, body: JSON.stringify(data) }).then(r => r.json()) as Promise<Offer>,
  updateOffer: (id: string, data: Partial<Offer>) =>
    fetch(`${API}/offers/${id}`, { method: "PUT", headers, body: JSON.stringify(data) }).then(r => r.json()) as Promise<Offer>,
  deleteOffer: (id: string) =>
    fetch(`${API}/offers/${id}`, { method: "DELETE", headers }).then(r => r.json()),
  getSettings: () => fetch(`${API}/settings`, { headers }).then(r => r.json()) as Promise<Partial<SiteSettings>>,
  saveSettings: (s: SiteSettings) =>
    fetch(`${API}/settings`, { method: "PUT", headers, body: JSON.stringify(s) }).then(r => r.json()) as Promise<SiteSettings>,
}
    ...
    <motion.div {...pageTransition}>
      ...
      <section className="relative min-h-[100svh] flex items-center justify-center overflow-hidden bg-[#050505] py-24 sm:py-28">
        ...
        >
          {/* Logo hero */}
          <motion.div
            className="flex justify-center mb-5 sm:mb-7 md:mb-8 relative"
            variants={fadeIn}
          >
            <div className="absolute inset-0 -m-10 sm:-m-16 pointer-events-none" style={{ background: "radial-gradient(circle, rgba(12,12,12,0.85) 20%, rgba(12,12,12,0.4) 55%, transparent 75%)" }} />
            <img
              src={logoImg as string}
              alt="AMC Auto Moto"
              className="h-[clamp(5.5rem,18vw,12rem)] w-[clamp(5.5rem,18vw,12rem)] max-h-[24svh] max-w-[24svh] object-contain relative z-10"
            />
          </motion.div>

          <motion.div
            variants={fadeUp}
            className="inline-flex max-w-full flex-wrap items-center justify-center gap-2 sm:gap-3 border border-[#c8102e] text-white text-[9px] sm:text-[10px] tracking-[0.22em] sm:tracking-[0.4em] uppercase px-4 sm:px-5 py-2.5 mb-7 sm:mb-10 bg-[#c8102e]"
          >
            <MapPin size={10} />
          ...
          <motion.h1
            variants={fadeUp}
            className="font-heading text-[clamp(2.35rem,9vw,7rem)] text-white font-bold leading-[0.98] tracking-tight mb-5 sm:mb-6 break-words"
          >
            Votre expert<br />
            <motion.span
          ...
          <motion.p variants={fadeUp} className="text-white/55 text-sm sm:text-base md:text-lg leading-relaxed max-w-xl mx-auto mb-8 sm:mb-12">
      ...
      >
        <div className="max-w-7xl mx-auto">
          ...
          <motion.p variants={fadeUp} className="text-white/25 text-xs text-center mt-6">
    ...
    <div className="pt-16">
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="border border-white/8 divide-y divide-white/8">
          {SERVICES.map(({ icon: Icon, title, short, details, img, price }) => (
            <div key={title} className="flex flex-col md:flex-row group hover:bg-white/[0.015] transition-colors duration-300">
              ...
              <div className="flex-1 p-8 md:p-10">
                <div className="flex items-start gap-5 mb-5">
                  <div className="w-11 h-11 flex items-center justify-center border border-[#c8102e]/35 shrink-0 mt-0.5 group-hover:border-[#c8102e] transition-colors duration-300">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      {price && (
                        <span className="shrink-0 border border-[#c8102e]/40 text-[#c8102e] text-[10px] font-bold tracking-[0.15em] uppercase px-3 py-1 mt-1">
                          {price}
                        </span>
                      )}
                    </div>
                    <p className="text-white/45 text-sm leading-relaxed mt-2">{short}</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-2 gap-x-4 ml-0 md:ml-16">
                  {details.map((d) => (
                    <div key={d} className="flex items-center gap-2 text-sm text-white/55">
                      <div className="w-1 h-1 bg-[#c8102e] shrink-0" />{d}
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="mt-3 border border-white/8 bg-[#0f0f0f] flex flex-col md:flex-row items-center gap-8 p-8 md:p-10">
          <div className="shrink-0 text-center md:text-left">
            ...
            <p className="font-heading text-xl text-[#c8102e] font-bold mt-1">Dès 69,90€ TTC</p>
          </div>
      ...
      <div className="flex-1 px-6 pb-24 max-w-7xl mx-auto w-full">
        {offers.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/8">
            {offers.map((offer) => (
              <div key={offer.id} className="bg-[#0c0c0c] flex flex-col">
                {offer.imageUrl && (
...
function AdminPage({ offers, setOffers, settings, setSettings, navigate }: {
  ...
  const [tab, setTab]           = useState<"offres" | "contact" | "textes">("offres")
  ...
  const inp = "w-full bg-[#0c0c0c] border border-white/10 text-white px-4 py-3 text-sm focus:border-[#c8102e]/60 focus:outline-none transition-colors placeholder:text-white/20"
  const lbl = "text-white/40 text-[10px] tracking-[0.2em] uppercase block mb-2"

  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault()
    if (pwd === ADMIN_PASSWORD) { setAuthed(true); setSettingsForm(settings) }
  ...
  const handleSaveOffer = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        const updated = await api.updateOffer(editing.id, offerForm)
        setOffers(offers.map(o => o.id === editing.id ? updated : o))
      } else {
        const created = await api.createOffer(offerForm)
  ...
  const handleDeleteOffer = async (id: string) => {
    ...
    setDeleting(id)
    try { await api.deleteOffer(id); setOffers(offers.filter(o => o.id !== id)) }
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      const saved = await api.saveSettings(settingsForm)
      setSettings(saved)
  ...
  return (
    <div className="pt-16 min-h-screen">
      {/* Header */}
      <div className="bg-[#111111] border-b border-white/8 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <h2 className="font-heading text-xl text-white font-bold tracking-tight">Admin AMC</h2>
          <div className="flex gap-1">
            {(["offres", "contact", "textes"] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-4 py-2 text-xs font-bold tracking-[0.15em] uppercase transition-colors ${tab === t ? "bg-[#c8102e] text-white" : "text-white/40 hover:text-white"}`}>
                {t === "offres" ? "Offres" : t === "contact" ? "Contact" : "Textes"}
              </button>
            ))}
          </div>
        </div>
        <div className="flex items-center gap-3">
          {tab === "offres" && (
            <button onClick={openNew} className="flex items-center gap-2 bg-[#c8102e] text-white px-4 py-2 font-bold tracking-[0.15em] uppercase text-xs hover:bg-[#a50d26] transition-colors">
              <Plus size={13} /> Nouvelle offre
            </button>
          )}
          <button onClick={() => navigate("accueil")} className="text-white/30 text-xs hover:text-white/60 transition-colors px-2">
            Voir le site
          </button>
          <button onClick={() => { setAuthed(false); navigate("accueil") }} className="text-white/30 hover:text-white/60 transition-colors" title="Déconnexion">
            <LogOut size={15} />
          </button>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        {tab === "contact" && (
          <form onSubmit={handleSaveSettings} className="space-y-8">
            <div className="bg-[#111111] border border-white/8 p-7 space-y-5">
              <h3 className="font-heading text-lg text-white font-bold mb-2">Coordonnées</h3>
            ...
            <div className="bg-[#111111] border border-white/8 p-7 space-y-4">
              <h3 className="font-heading text-lg text-white font-bold mb-2">Horaires d'ouverture</h3>
              ...
              <button type="button" onClick={() => setSettingsForm(p => ({ ...p, hours: [...p.hours, { day: "", hours: "" }] }))}
                className="text-white/30 text-xs hover:text-white transition-colors flex items-center gap-1 mt-2">
        ...
        {tab === "textes" && (
          <form onSubmit={handleSaveSettings} className="space-y-8">
            <div className="bg-[#111111] border border-white/8 p-7 space-y-5">
              <h3 className="font-heading text-lg text-white font-bold mb-2">Page d'accueil</h3>
              <div>
                <label className={lbl}>Phrase d'accroche (sous le titre)</label>
                <textarea rows={2} value={settingsForm.heroSubtitle} onChange={e => setSettingsForm(p => ({ ...p, heroSubtitle: e.target.value }))} className={`${inp} resize-none`} />
              </div>
            </div>

            <div className="bg-[#111111] border border-white/8 p-7 space-y-5">
              <h3 className="font-heading text-lg text-white font-bold mb-2">Page À propos</h3>
              <div><label className={lbl}>Paragraphe 1</label><textarea rows={3} value={settingsForm.aboutText1} onChange={e => setSettingsForm(p => ({ ...p, aboutText1: e.target.value }))} className={`${inp} resize-none`} /></div>
              <div><label className={lbl}>Paragraphe 2</label><textarea rows={3} value={settingsForm.aboutText2} onChange={e => setSettingsForm(p => ({ ...p, aboutText2: e.target.value }))} className={`${inp} resize-none`} /></div>
              <div><label className={lbl}>Paragraphe 3</label><textarea rows={3} value={settingsForm.aboutText3} onChange={e => setSettingsForm(p => ({ ...p, aboutText3: e.target.value }))} className={`${inp} resize-none`} /></div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-24">
          ...
          <div>
            <h2 className="font-heading text-4xl md:text-5xl text-white font-bold tracking-tight leading-tight mb-7">
              Un garage de confiance depuis plus de 20 ans
            </h2>
...
function ContactPage({ settings }: { settings: SiteSettings }) {
  ...
  const inputCls = "w-full bg-[#0c0c0c] border border-white/10 text-white px-4 py-3 text-sm focus:border-[#c8102e]/60 focus:outline-none transition-colors placeholder:text-white/20"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col gap-6">
            <div className="bg-[#111111] border border-white/8 p-8">
              <h3 className="font-heading text-xl text-white font-bold mb-7 tracking-tight">Informations pratiques</h3>
              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  <div className="w-8 h-8 flex items-center justify-center border border-[#c8102e]/35 shrink-0">
                    <MapPin size={14} className="text-[#c8102e]" />
                  </div>
                  <div>
                    <p className="text-white/70 text-sm font-medium mb-0.5">Adresse</p>
                    <p className="text-white/40 text-sm">{settings.address}<br />{settings.postalCode} {settings.city}</p>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  <div className="w-8 h-8 flex items-center justify-center border border-[#c8102e]/35 shrink-0">
                  ...
                  <div>
                    <p className="text-white/70 text-sm font-medium mb-0.5">Téléphone</p>
                    <a href={`tel:${settings.phone.replace(/\s/g, "")}`} className="text-white/40 text-sm hover:text-[#c8102e] transition-colors block">
                      {settings.phone}
                    </a>
                    {settings.phone2 && (
                      <a href={`tel:${settings.phone2.replace(/\s/g, "")}`} className="text-white/40 text-sm hover:text-[#c8102e] transition-colors block">
                        {settings.phone2}
                      </a>
                    )}
                  </div>
                ...
                {settings.email && (
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-white/70 text-sm font-medium mb-0.5">E-mail</p>
                      <a href={`mailto:${settings.email}`} className="text-white/40 text-sm hover:text-[#c8102e] transition-colors">
                        {settings.email}
                      </a>
                    </div>
                  </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-white/70 text-sm font-medium mb-2">Horaires d'ouverture</p>
                    <div className="space-y-1.5">
                      {settings.hours.map(({ day, hours }) => (
                        <div key={day} className="flex justify-between gap-8 text-sm">
                          ...
                          <span className={hours === "Fermé" ? "text-[#c8102e]/80" : "text-white/55"}>{hours}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {/* Map */}
            <div className="overflow-hidden border border-white/8 h-60 bg-[#1a1a1a]">
              <iframe
          ...
          {/* Form */}
          <div className="bg-[#111111] border border-white/8 p-8">
            <h3 className="font-heading text-xl text-white font-bold mb-7 tracking-tight">Envoyer un message</h3>
            {sent ? (
              <div className="flex flex-col items-center justify-center h-72 text-center">
                <div className="w-14 h-14 flex items-center justify-center border border-[#c8102e]/40 mb-5">
                  <CheckCircle size={24} className="text-[#c8102e]" />
                </div>
                <h4 className="font-heading text-2xl text-white font-bold mb-2">Message envoyé</h4>
                <p className="text-white/40 text-sm">Nous vous répondrons dans les plus brefs délais.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-white/35 text-[10px] tracking-[0.2em] uppercase block mb-2">Nom *</label>
                    <input
                      type="text"
                      required
                      value={form.name}
                  ...
                  <div>
                    <label className="text-white/35 text-[10px] tracking-[0.2em] uppercase block mb-2">Téléphone</label>
                    <input
                      type="tel"
                      value={form.phone}
                ...
                <div>
                  <label className="text-white/35 text-[10px] tracking-[0.2em] uppercase block mb-2">E-mail *</label>
                  <input
                    type="email"
                    required
                    value={form.email}
                ...
                <div>
                  <label className="text-white/35 text-[10px] tracking-[0.2em] uppercase block mb-2">Message *</label>
                  <textarea
                    required
                    rows={5}
                    value={form.message}
                    ...
                    placeholder="Décrivez votre besoin (véhicule, type d'intervention...)..."
                  />
                </div>
                <button
                  type="submit"
                  className="w-full bg-[#c8102e] text-white py-4 font-bold tracking-[0.2em] uppercase text-xs hover:bg-[#a50d26] transition-colors duration-200"
                >
                  Envoyer le message
                </button>
              </form>
